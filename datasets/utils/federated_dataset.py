...
def partition_digits_domain_label_skew_loaders(train_datasets: list, test_datasets: list,
                                         setting: FederatedDataset) -> Tuple[list, list]:
    ini_len_dict = {}
    not_used_index_dict = {}
    net_dataidx_map={}
    ...
    for index in range(len(train_datasets)):
        ...
        # print(f"Domain {name}: selected {len(selected_idx)} samples")

        # y_train = np.array(train_dataset.targets)[selected_idx]
        if name == 'svhn':
            train_dataset = train_datasets[index].dataset
            y_train = train_dataset.labels
        elif name in ['syn', 'mnist-m']:
        ...
        while min_size < min_require_size:
            ...
            for k in range(n_class):
                ...
                np.random.shuffle(idx_k)
                # if n_class_sample != None:
                #     idx_k = idx_k[:n_class_sample * n_participants]

                if gamma == 0:
                    idx_batch = [idx_j + idx.tolist() for idx_j, idx in
                                 zip(idx_batch, np.array_split(idx_k, n_participants))]
                else:
                    proportions = np.random.dirichlet(np.repeat(gamma, repeats=n_participants))
                    proportions = np.array(
                        [p * (len(idx_j) < len(y_train) / n_participants) for p, idx_j in zip(proportions, idx_batch)])
                    proportions = proportions / proportions.sum()
        ...
        for j in range(n_participants):
            np.random.shuffle(idx_batch[j])
            # if n_class_sample != None:
            #     idx_batch[j] = idx_batch[j][:n_class_sample * n_class]
            net_dataidx_map[j] = idx_batch[j]
...
def partition_vlcs_skew_loaders(train_datasets: list, test_datasets: list,
                                setting: FederatedDataset) -> Tuple[list, list]:
    ini_len_dict = {}
    not_used_index_dict = {}
    # all_labels = []
    for i in range(len(train_datasets)):
        name = train_datasets[i].data_name
        all_train_index = np.array(train_datasets[i].train_data_list)
    ...
    for index in range(len(test_datasets)):
        ...
        test_loader = DataLoader(test_dataset, batch_size=setting.args.local_batch_size, shuffle=False)
        setting.test_loader.append(test_loader)
...
def partition_office_domain_skew_loaders_new(train_datasets: list, test_datasets: list,
    ...
    for index in range(len(train_datasets)):
        ...
        y_train = np.array(train_labels)[selected_idx]
        train_sampler = SubsetRandomSampler(selected_idx)
...
def partition_pacs_skew_loaders(train_datasets: list, test_datasets: list,
                                         setting: FederatedDataset) -> Tuple[list, list]:
    ini_len_dict = {}
    not_used_index_dict = {}
    # all_labels = []
    for i in range(len(train_datasets)):
        ...
        if name not in not_used_index_dict:
            ...
            y_train= train_datasets[i].labels
            not_used_index_dict[name] = np.arange(len(y_train))
            ini_len_dict[name] = len(y_train)
...
def partition_domainnet_domain_skew_loaders(train_datasets: list, test_datasets: list,
                                             setting: FederatedDataset) -> Tuple[list, list]:
    ini_len_dict = {}
    not_used_index_dict = {}
    all_labels_list = []
